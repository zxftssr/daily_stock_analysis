...
条件运算符固定为 `lte`（小于等于）、`gte`（大于等于）和 `between`（进入区间），不支持自由公式或脚本。活跃计划作为独立的每日职责执行；即使 AI 分析、大盘复盘或合并通知失败，也会继续检查当日开市市场的计划。计划检查自身采用 fail-open，不影响原报告、通知或回测。手工点击「检查」不会发送通知，每日任务只对尚未通知的首次触发档位走 `alert` 通知路由；发送失败会保留待通知状态供下次重试。系统不会自动下单。
...
计划生命周期为 `draft -> active -> paused/closed`。活跃计划必须先暂停才能替换执行档位；已触发档位可手工标记为完成、跳过，也可在未执行时直接重置为待触发。已完成或已跳过档位是不可改写的执行历史；如需调整档位，应关闭旧计划并新建计划。关闭计划后不可重新激活。
...
| `/api/v1/investment-plans/{id}/evaluate` | POST | 手工检查一份活跃计划，不发送通知 |
