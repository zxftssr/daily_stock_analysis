...
计划还可独立开启或关闭“触发时通知”。通知渠道留空时沿用全局 `alert` 路由，也可指定一个已在设置中配置的静态渠道。自动检查和 Web 单项手工检查都会对尚未通知的首次触发档位发送提醒；发送失败会保留待通知状态供下次重试，同一档位发送成功后不会重复通知。系统不会自动下单。
手工单项检查遇到行情不足时显示「行情数据不足」及具体原因；批量检查会将「数据不足」与异常失败分别计数，不会把未完成的条件校验显示成成功无触发。
...
| `/api/v1/investment-plans/{id}/evaluate?notify=false` | POST | 手工检查一份活跃计划；显式 `notify=true` 时按该计划的通知设置尝试发送 |
| `/api/v1/investment-plans/evaluate-active?notify=false` | POST | 批量检查活跃计划；仅显式 `notify=true` 时尝试发送 alert 通知 |
