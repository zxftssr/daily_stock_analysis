...
- `benchmark_drawdown_250d_pct`：对标指数最近 250 根日 K 相对区间高点的回撤百分比，内部请求 550 个自然日窗口以覆盖至少 250 个完整交易日；K 线必须更新至最近已完成交易日，过期行情不会触发。盘中高频检查会用本轮实时价格与 250 日高点计算当前回撤，实时价格不可验证时不触发。使用该指标时必须填写对标指数。

条件运算符固定为 `lte`（小于等于）、`gte`（大于等于）和 `between`（进入区间），不支持自由公式或脚本。每份计划可选择 `minute`（盘中高频，每分钟）、`daily`（每日定时任务）、`hourly`（每小时）或 `manual`（仅手工）检查频率。盘中高频当前只支持能够提供可信报价时间的 A 股与港股；美股计划可使用每小时、每日或手工检查。每分钟与每小时任务只在 `schedule` 模式运行；盘中高频任务会跳过缺少供应商报价时间的数据源，继续尝试后续来源，并按交易所日历和实际评估时刻过滤市场，包括午间休市。当前没有开市市场时不会请求行情，日历不可用或无法确认时停止本轮高频检查。每日分析在独立的非重入线程中运行，不会阻塞每分钟检查；如果上一轮每日分析仍未结束，新的每日触发会跳过。从 ETF 回测一键创建的新大跌计划默认使用盘中高频检查。即使 AI 分析、大盘复盘或合并通知失败，自动计划检查仍会独立执行，并只处理当前符合市场与频率条件的计划。计划检查自身采用 fail-open，不影响原报告、通知或回测。

计划还可独立开启或关闭“触发时通知”。通知渠道留空时沿用全局 `alert` 路由，也可指定一个已在设置中配置的静态渠道。自动检查和 Web 单项手工检查都会对尚未通知的首次触发档位发送提醒；Web 手工检查会先返回已经落库的检查结果，再在响应后发送通知，页面以“通知已进入后台发送”区分检查结果与送达状态。发送失败会保留待通知状态供下次重试，同一档位发送成功后不会重复通知。系统不会自动下单。
