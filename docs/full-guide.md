...
- `benchmark_drawdown_250d_pct`：对标指数最近 250 根日 K 相对区间高点的回撤百分比，内部请求 550 个自然日窗口以覆盖至少 250 个完整交易日；K 线必须更新至最近已完成交易日，过期行情不会触发。盘中高频检查与回测统一使用 250 个交易时段：249 根已完成日 K 加当前交易日，当日高点优先取可验证实时行情的最高价，缺失时以当前价作为保守下界；实时价格不可验证时不触发。美股分钟检查仅在需要可验证时间戳时请求 Yahoo 1 分钟 K 线，使用 K 线自身的美东时间并使用 15 秒短缓存；过期或休市 K 线会被后续日历校验拒绝。使用该指标时必须填写对标指数。

条件运算符固定为 `lte`、`gte` 和 `between`。每份计划可选择 `minute`、`daily`、`hourly` 或 `manual`。A 股、港股和美股均支持盘中每分钟检查；美股使用 Yahoo 1 分钟 K 线时间戳。高频任务只在 `schedule` 模式运行，会跳过缺少时间戳的来源，并同时校验报价时刻、检查时刻和交易所日历；休市、过期或无法确认的行情不会触发。每日分析与分钟检查互不阻塞，自动计划检查失败也不影响原报告、通知或回测。
