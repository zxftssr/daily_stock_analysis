...
- `price`：标的最新价格；策略检查使用只获取价格的轻量行情路径，不为条件判断拉取量比、估值等增强字段。行情日期必须覆盖该市场最近已完成交易日；供应商返回可信报价时间时直接使用该价格，实时报价缺失、为 0、时间无法验证或已经过期时，系统改用经交易日校验的最新日 K 收盘价；
- `benchmark_drawdown_250d_pct`：对标指数最近 250 根日 K 相对区间高点的回撤百分比，内部请求 550 个自然日窗口以覆盖至少 250 个完整交日；K 线必须更新至最近已完成交易日，过期行情不会触发。使用该指标时必须填写对标指数。

条件运算符固定为 `lte`（小于等于）、`gte`（大于等于）和 `between`（进入区间），不支持自由公式或脚本。每份计划可选择 `daily`（每日定时任务）、`hourly`（每小时）或 `manual`（仅手工）检查频率；每小时任务只在 `schedule` 模式运行。即使 AI 分析、大盘复盘或合并通知失败，自动计划检查仍会独立执行，并只处理当日开市市场与对应频率的计划。计划检查自身采用 fail-open，不影响原报告、通知或回测。
