...
- 免费，无需配置
- 数据来源：东方财富爬虫
- 港股实时行情的 `stock_hk_spot_em()` / `stock_hk_spot()`，以及 A 股大盘统计的 `stock_zh_a_spot_em()` / `stock_zh_a_spot()`，均有调用方超时保护：默认最多等待 30 秒，超时后当前问股、分析或市场复盘会继续走备用链路或降级返回
- AkShare 调用超时后，同一函数会进入约 60 秒冷却期；若底层线程仍未返回，不会重复提交同名调用。后台 AkShare 调用池最多保留 2 个工作线程，池满时直接降级，避免第三方接口卡死后继续阻塞定时调度
...
为减少单一区间偶然性，页面还提供固定参数的「稳健性验证」。系统按照用户设置的窗口交易日数和滚动步长，在主 ETF 及最多 4 只对照 ETF 上重复执行完全相同的档位配置；不会自动搜索、排序或替换用户参数。系统先按交易日划出样本内/样本外日期边界，再在两侧分别生成窗口，任何窗口都不会跨越边界；当指定比例不足以容纳一个完整窗口时，会把该侧扩展到至少一个窗口所需的交易日数。

每个窗口可按最低收益、最大回撤和最少触发档位判定通过。每只 ETF 至少保留一个样本内和一个样本外窗口，同一 ETF 的代码别名只计一次；单次请求最多展开 500 个窗口，超限时需增大步长或缩短日期范围。汇总同时展示总体与样本外通过率、平均/中位/最差收益、最差最大回撤、平均资金利用率及档位触发覆盖。只有有效窗口数达到门槛、总体与样本外通过率均达标、且所有所选 ETF 都有可用历史窗口时，当前参数才通过稳健性验证并开放「一键创建并启用策略」；任一回测参数或稳健性门槛变化都会使旧结论失效。

---

## 投资策略计划

Web「策略计划」页用于把用户已经形成的投资判断写成可检查的执行纪律。第一版支持指数大跌、波段交易、股息收息、周期布局、价值投资、成长投资六类计划模板；策略类型用于分类和呈现，不代表系统已经自动验证公司质量或行业周期。

选择模板时，编辑器会填入对应的投资逻辑、失效条件、仓位/现金纪律和执行档位脚手架。指数大跌模板默认使用沪深300的 20%/30% 回撤分批档位；其余模板保留需用户根据标的估值填写的价格阈值。模板只是可编辑起点，激活前应根据自己的研究修订。

每份计划包含投资逻辑、失效条件、可选账户、最大仓位、最低现金比例、复查日期和一组执行档位。执行档位只允许以下确定性指标：

港股代码支持 `700`、`00700`、`HK00700` 和 `00700.HK` 等输入，计划内统一存储为 `HK00700`；列表以 1–5 位纯数字筛选时使用相同标准化规则。
显式携带 `SH` / `SZ` 的 A 股代码会保留必要的证券身份区分，但价格与回撤时效校验仍统一使用 A 股最近已完成交易日。
`HSI` / `HSCEI` / `HSTECH` 统一按港股交易日历校验，`AAPL.US` 等 `.US` 形式按美股交易日历校验。持仓中的 `00700` / `HK00700` 与 `AAPL` / `AAPL.US` 在仓位纪律比较时视为同一证券。
Provider 历史数据路由与日历使用同一规则：上述港股指数别名会跳过可能误解为美股的公开行情源，直接交给 Yahoo 的 `^HSI` / `^HSCE` / `HSTECH.HK`；`.US` 后缀会在路由前移除。
Yahoo Finance 历史数据适配器会将项目内部的“包含式结束日”转换为 Yahoo 所需的下一自然日排除式参数，避免 SPX / NDX 等指数少最后一根 K 线。

- `price`：标的最新价格；策略检查使用只获取价格的轻量行情路径，不为条件判断拉取量比、估值等增强字段。行情日期必须覆盖该市场最近已完成交易日；供应商返回可信报价时间时直接使用该价格。普通检查在实时报价缺失、为 0、时间无法验证或已经过期时，会改用经交易日校验的最新日 K 收盘价；盘中高频检查要求报价时间带时区、属于有效交易时段且距离本轮检查不超过 5 分钟，不会用日线收盘价代替实时价，以免误报；
- `benchmark_drawdown_250d_pct`：对标指数最近 250 根日 K 相对区间高点的回撤百分比，内部请求 550 个自然日窗口以覆盖至少 250 个完整交易日；K 线必须更新至最近已完成交易日，过期行情不会触发。盘中高频检查与回测统一使用 250 个交易时段：249 根已完成日 K 加当前交易日，当日高点优先取可验证实时行情的最高价，缺失时以当前价作为保守下界；实时价格不可验证时不触发。使用该指标时必须填写对标指数。无后缀的沪深 300 代码 `000300` 会按沪市指数处理。

条件运算符固定为 `lte`、`gte` 和 `between`。每份计划可选择 `minute`、`daily`、`hourly` 或 `manual`。盘中每分钟检查当前支持 A 股和港股；美股免费分钟行情在真实交易时段可能被限流，因此在接入可靠实时行情源前不开放 `minute`。升级时，SQLite 中已存在的美股 `minute` 计划会幂等迁移为 `daily`，原状态、执行档位和资金记录保持不变，活跃计划会继续参与每日调度。高频任务只在 `schedule` 模式运行，会跳过缺少时间戳的来源，并同时校验报价时刻、检查时刻和交易所日历；休市、过期或无法确认的行情不会触发。每日分析与分钟检查互不阻塞，自动计划检查失败也不影响原报告、通知或回测。配置 `TUSHARE_TOKEN` 不会自动把 Tushare 加入实时行情优先级；需要时须在 `REALTIME_SOURCE_PRIORITY` 中显式列出。

独立 `schedule` 进程会在数据库同目录原子写入 `scheduler_status.json`，每个调度循环更新一次心跳，并记录最近一轮分钟检查的市场、完成状态、计划数、新触发数和错误数。Web 策略计划页通过 `GET /api/v1/investment-plans/scheduler-status` 展示调度服务在线状态、最近心跳与最近分钟检查；心跳超过 90 秒即显示离线，进程正常退出会立即显示已停止。状态文件损坏或暂时无法读取只影响可观测性，不会中断调度任务。Docker 的 `analyzer` 与 `server` 已共享 `/app/data`，无需新增配置；自定义部署必须确保两端的 `DATABASE_PATH` 指向同一共享目录。

计划还可独立开启或关闭“触发时通知”。通知渠道留空时沿用全局 `alert` 路由，也可指定一个已在设置中配置的静态渠道。自动检查和 Web 单项手工检查都会对尚未通知的首次触发档位发送提醒；Web 手工检查会先返回已经落库的检查结果，再在响应后发送通知，页面以“通知已进入后台发送”区分检查结果与送达状态。每个档位会持久化展示“待发送 / 已发送 / 发送失败，待重试 / 渠道不可用，待重试”，并显示最近状态时间和失败原因。发送失败会保留待通知状态供下次重试，同一档位发送成功后不会重复通知。系统不会自动下单。
手工单项检查使用独立的 90 秒请求时限，批量检查使用 180 秒；遇到行情不足时显示「行情数据不足」及具体原因。单项检查只会在成功读取最新检查时间作为基线后发起；若浏览器等待超时且后台尚未落库，按钮会变为「重新确认」，只读取状态而不会重复检查。检测到新的 `lastEvaluatedAt` 后，单项与批量检查按钮会在当前页面恢复。批量检查会将「数据不足」与异常失败分别计数，不会把未完成的条件校验显示成成功无触发。
检查使用计划快照计算；如果在获取行情/持仓期间计划或档位被另一请求修改，本次旧快照结果会拒绝落库，不会覆盖新状态。

计划生命周期为 `draft -> active -> paused/closed`。Web 将不可恢复的 `closed` 操作显示为“移除”；计划及执行历史仍会保留。列表默认选择“当前计划”并隐藏已移除记录，主动选择“全部状态”或“已移除”后仍可查看历史。活跃计划必须先暂停才能替换执行档位；已触发档位可手工标记为完成、跳过，也可在未执行时直接重置为待触发。已完成或已跳过档位是不可改写的执行历史；如需调整档位，应移除旧计划并新建计划。已移除计划不可重新激活。

指数大跌计划支持独立的人工成交与复盘闭环，不要求绑定持仓账户。创建计划时可填写“计划资金”；从 ETF 回测一键创建时会自动继承回测初始资金。所有金额按标的市场本币计价，Web 会明确显示 `CNY`、`HKD` 或 `USD`，不会进行汇率换算。买入/加仓档位触发后，Web 以“登记买入”代替普通完成操作，要求填写真实成交时间（精确到秒）、价格、份额、手续费和可选备注，成交金额由价格与份额计算；浏览器会同时提交当时的 UTC 偏移，后端统一换算到服务端时区，API 不接受只有日期、缺少秒或缺少时区的时间。登记成功后该档位不可改写，系统按全部已登记档位汇总累计投入、持有份额、含费平均成本、剩余现金、最新估值、浮动盈亏、实际收益和计划资金占用率；最新估值会比较完整的计划检查时间与实际成交时间，延迟登记或历史补录不会把旧成交价伪装成最新价格事件，同日后续检查也会采用更新的检查价。升级前只有成交日期、没有精确时间的记录按日期比较：更晚日期的检查优先，同日保持成交价这一保守回退。未绑定账户时，计划资金占用率还会与当前档位目标比较，建议新增金额按“当前计划权益 × 目标比例 − 当前市值”计算；绑定账户时，档位目标仍是账户总权益口径，页面会明确标为独立口径，不计算偏差，也不自动建议金额。成交总成本不能超过计划剩余现金；已有成交后计划资金不可修改，计划激活后也不能再更换策略类型。升级前已经完成但没有成交明细的指数大跌档位会标记为“待补录”，补录完成前不计算余额、收益和仓位，也不能登记新的触发档位；已移除的旧计划也允许仅对这些缺失记录执行一次补录，若未设置计划资金则保留成交成本，但余额与占用率继续显示为不可计算。跳过与未执行重置仍沿用原档位操作。该台账仅记录用户确认的券商成交，不连接券商，也不会自动下单。

当计划绑定持仓账户时，买入/加仓触发会同时检查当前个股仓位和现金比例。如档位填写了“执行后目标仓位”，系统还会估算从当前仓位增加到目标仓位后的现金比例；当前仓位已达到某档目标时，该档不会触发，但更高目标档仍可正常评估。达到最大仓位、当前或预计现金跌破底线，或账户快照、汇率、任一持仓价格缺失/过期时，档位仍记录为已触发，但买入纪律会阻止，要求用户人工处理。同时命中卖出/复查与受限买入时，整体状态仍是“已触发”，但 alert 会明确列出买入受限原因。
持仓价格的“过期”也按该证券市场的最近完整交易日复核：例如美股盘前的上一交易日收盘价仍属于可靠估值，不会因中国自然日已翻页而被误阻止。

主要 API：

| 接口 | 方法 | 说明 |
|------|------|------|
| `/api/v1/investment-plans` | GET/POST | 查询或创建计划 |
| `/api/v1/investment-plans/{id}` | GET/PUT | 查看或修改计划 |
| `/api/v1/investment-plans/{id}/status` | PATCH | 激活、暂停或关闭计划 |
| `/api/v1/investment-plans/{id}/steps/{step_id}` | PATCH | 完成或跳过已触发档位；未执行前可直接重置已触发档位 |
| `/api/v1/investment-plans/{id}/evaluate?notify=false` | POST | 手工检查一份活跃计划；显式 `notify=true` 时检查先返回，通知在响应后发送 |
| `/api/v1/investment-plans/evaluate-active?notify=false` | POST | 批量检查活跃计划；显式 `notify=true` 时检查先返回，alert 通知在响应后发送 |
...
- 🎨 **浅色优先工作台** - Web 全站采用中性、紧凑的 shadcn/ui 风格，默认浅色并保留深色与跟随系统主题；桌面侧栏按功能分组并可折叠，折叠后的路由、主题、退出和展开图标在鼠标悬停或键盘聚焦时均显示可见提示
- 📝 **配置管理** - 查看/修改自选股列表
...
- 🔎 **标的发现** - Web 端可在股票和 A 股宽基 ETF 之间切换；股票支持市场、行业和关键词筛选，宽基 ETF 支持精选池、跟踪指数、250 日回撤及 20/60/250 日收益榜，并可查看 K 线、分析、问股、加入自选或一键制定指数大跌计划
- 💡 **推荐关注候选池** - Web「候选」页基于静态股票索引和行情榜单做规则评分，可按市场、行业、关键词和候选模式筛选候选股票，并一键进入自选、K 线、分析或问股
- 🤖 **历史报告追问上下文** - 首页历史报告的「追问 AI」会创建新的问股会话，在消息流中展示可展开的报告来源卡片，刷新、服务重启或切换历史会话后仍可恢复，并持续注入给后续追问
- 📈 **回测验证** - 评估历史分析准确率，查询方向胜率与模拟收益
- 📋 **策略计划** - 记录投资逻辑、失效条件、仓位纪律和价格/指数回撤档位，支持手工检查、每日检查与首次触发提醒
...
| `/api/v1/backtest/etf-crash/robustness` | POST | 使用固定参数执行多区间、跨 ETF 的滚动稳健性验证 |
| `/api/v1/investment-plans` | GET/POST | 查询或创建投资策略计划 |
| `/api/v1/investment-plans/{id}/steps/{step_id}/execution` | POST | 为已触发的指数大跌买入/加仓档位登记人工成交并返回持仓复盘 |
...
| `/api/v1/stocks/rankings?market=CN|BSE|HK|US&asset_type=stock|etf&metric=...&direction=desc|asc` | GET | 查询股票或宽基 ETF 榜单；ETF 当前仅支持 CN，可按 `category` 过滤并使用 `drawdown_250d_pct`、`return_20d_pct`、`return_60d_pct`、`return_250d_pct` 排序 |
| `/api/v1/stocks/etf-history/warmup?force_refresh=true` | POST | 立即预热宽基 ETF 精选池日线并写入 SQLite，返回成功、旧缓存和失败明细 |
| `/api/v1/stocks/{stock_code}/history?period=daily&days=30&force_refresh=false` | GET | 查询个股日 K 历史数据，Web 发现页 K 线抽屉复用该接口，`days` 表示自然日窗口，可选 30/90/180/365；响应包含 `source/cache_hit/stale/as_of_date/message` 等缓存元信息 |
...
> 说明：A 股宽基 ETF 精选池以 `data/cn_broad_etf_pool.csv` 为唯一源数据，由 `scripts/generate_index_from_csv.py` 合并进 `stocks.index.json`。当前池包含 20 只成交活跃产品，覆盖沪深300、中证A500、中证500、中证1000、中证2000、上证50、深证100、创业板、创业板50、科创50及科创100；同指数存在多只产品时优先纳入近期成交活跃的产品。池内只静态保存代码、名称、分类、跟踪指数及展示优先级，不保存实时价格、基金规模或费率；实际成交额和成交量仍按需通过 efinance 获取并以 AkShare 兜底，可在发现页动态排序。每日调度任务会在分析和策略检查前批量预热精选池日线，发现页也提供「立即预热」；数据复用通用 `stock_daily` SQLite 表，单只失败不阻断批次。250 日回撤和 20/60/250 日收益由发现榜和指数大跌策略共用同一计算服务，页面展示历史覆盖率、共同截至日期和旧缓存状态；旧缓存仍可用于观察，但不会触发策略档位。发现页的「制定计划」会预填 `index_crash` 模板，并用 ETF 自身作为回撤检查基准；计划仍可不绑定账户，达到档位后复用计划通知链路。
> 说明：`stocks.index.json` 的 `popularity` 是离线静态热度分，范围 `0..100`，用于发现页/候选池排序参考，不是买入评级、投资建议或个人偏好。可通过 `python3 scripts/update_stock_popularity.py --test` 查看覆盖率与样例，通过 `python3 scripts/update_stock_popularity.py` 正式刷新 `data/stock_popularity_cache.json` 并原子写回索引；默认覆盖 `CN,BSE,HK,US`，美股只刷新 `data/us_ranking_core_pool.csv` 核心池，全量美股需显式传 `--us-scope all`。默认核心池模式只更新核心池股票，核心池外美股会保留原索引分数；局部 `--markets` 刷新也只触达请求市场。A 股/北交所会先尝试现有受保护批量行情源，失败后再用 Eastmoney 轻量分页 fallback 只抓评分必需字段，降低大批量接口被断开时的整市场失败概率。免费数据源覆盖和稳定性会波动：市场级数据源失败、或数据源只返回空壳指标且没有有效缓存时，脚本默认保留该市场原有分数，只有显式 `--allow-zero-write` 才会批量写 `0`；北交所低覆盖默认只告警不阻断。当前 popularity 刷新脚本不接入 Tushare 补充路径，若需要更高覆盖率可先刷新 `data/stock_list_*.csv` 或后续扩展付费/权限源。
> 说明：Web「候选」页第一版不新增后端候选池 API，也不接入 LLM 推荐。页面先以 `stocks.index.json` 的市场、行业、关键词筛选结果作为候选全集，再按「综合关注 / 趋势活跃 / 成交活跃 / 回调观察」模式串行读取必要的 `GET /api/v1/stocks/rankings` 榜单信号；浏览器会对同一市场、行业和候选模式的榜单信号保留 5 分钟会话缓存，刷新页面时先展示缓存候选并在后台更新。仅当主榜单返回 `unavailable` / `unsupported` 且没有任何条目时，才会停止后续榜单请求，避免冷缓存或未支持市场放大外部行情源请求。榜单只给静态候选内已有股票叠加价格、涨跌幅、成交额/成交量和事实型理由，不会把不匹配关键词或筛选条件的榜单股票扩张进候选列表。候选页状态会区分实时、部分行情、缓存行情和静态候选；所有榜单不可用时仍可按静态热度展示候选。关注分仅表示“值得进一步查看的信号强度”，不是买入评级或投资建议；只有用户主动点击星标后，股票才会写入 `STOCK_LIST` 并进入后续定时分析与通知流程。
