...
计划生命周期为 `draft -> active -> paused/closed`。Web 将不可恢复的 `closed` 操作显示为“移除”；计划及执行历史仍会保留。活跃计划必须先暂停才能替换执行档位；已触发档位可手工标记为完成、跳过，也可在未执行时直接重置为待触发。已完成或已跳过档位是不可改写的执行历史；如需调整档位，应移除旧计划并新建计划。已移除计划不可重新激活。
