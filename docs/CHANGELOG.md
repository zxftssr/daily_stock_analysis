...
- [修复] 档位完成/跳过或计划移除后清除未送达通知的待重试状态，已送达历史仍保留，避免页面显示永久无法执行的重试提示
- [改进] 盘中批量轮询直接复用已按市场和频率筛选的计划快照，避免每份计划再执行一次 SQLite 详情查询，同时保留落库时的版本冲突校验
- [新功能] 策略档位持久化展示通知待发送、已发送、发送失败或渠道不可用状态，并记录最近状态时间与失败原因供页面查看和后续重试
- [修复] ETF 盘中回撤与回测统一为 250 个交易时段口径，使用 249 根已完成日 K 加当日实时价/最高价，避免盘中检查误纳入 251 个时段
- [新功能] 策略计划页新增独立 schedule 服务状态面板，展示在线/离线、最近心跳、每日时间及最近分钟检查的市场、计划数、触发数和错误数
- [改进] schedule 进程在数据库同目录原子维护跨进程心跳文件，心跳写入或状态读取失败不影响分析与策略检查，Docker analyzer/server 可直接通过共享 data 卷联动
- [测试] 增加调度器心跳启动、超时、停止、重启恢复、损坏文件降级、API 合约及 Web 状态展示回归测试
- [新功能] 投资策略计划新增盘中高频检查，在 schedule 模式下按对应市场交易时段每分钟检查一次，ETF 大跌计划使用实时价计算当前 250 日回撤并在首次触发后立即通知
- [改进] ETF 回测一键创建的新策略默认使用盘中高频检查；实时行情不可验证时不以日线收盘价代替，避免高频档位误报
- [测试] 增加盘中市场时段、每分钟调度、minute API/Web 合约、实时回撤与无实时价不触发回归测试
- [修复] 盘中高频报价增加交易时段与 5 分钟新鲜度校验，交易日历异常时停止本轮检查，并将每日分析移入非重入 worker，避免陈旧报价误报或长分析阻塞分钟调度
- [修复] 盘中高频取价会跳过无供应商时间戳的来源，并在落库前再次确认市场仍处于交易时段；A 股/港股使用现有时间戳来源，美股使用 Yahoo 1 分钟 K 线时间戳
- [修复] DatabaseManager 首次懒初始化增加线程互斥与失败实例清理，避免冷启动并发请求拿到半初始化连接并偶发返回 500
- [改进] 投资策略计划价格检查改用轻量实时行情路径，首个有效价格即可返回，避免为未使用的 ETF 估值字段拉取全市场快照
- [修复] 实时行情透传供应商报价时间并修正补充源次数上限；无可信时间时仍安全回退日线，避免把请求时间伪装成行情日期
- [新功能] ETF 指数大跌计划新增人工成交登记与复盘闭环，支持计划资金、成交日期/价格/份额/手续费台账，以及累计投入、平均成本、剩余现金、仓位偏差和浮动收益核算
- [改进] ETF 回测一键创建策略会继承初始资金，已触发买入档位必须登记真实成交后才完成，系统仍不连接券商或自动下单
- [修复] SQLite 启动升级幂等补齐策略计划资金和成交台账字段，保留既有计划与档位数据
- [测试] 增加 ETF 成交登记服务/API/Web 闭环、剩余资金约束、不可改写计划资金及旧库升级场景测试
- [修复] 旧版已完成 ETF 档位增加成交待补录门禁，绑定账户与计划资金仓位口径分离，并冻结已激活计划的策略类型以防绕过成交登记
- [修复] ETF 成交复盘按检查日与成交日选择更新估值价格，未绑定账户的建议金额改按当前计划权益计算，绑定账户时不再跨口径自动建议
- [修复] ETF 成交复盘改用完整检查时间与成交时间判断同日价格先后，并按市场本币显示 CNY/HKD/USD，避免旧成交价和人民币符号污染复盘
- [修复] ETF 成交新增带时区的实际成交时间并用于估值先后判断，API 拒绝缺少秒或时区的时间；历史补录不再以录入时间覆盖更新检查价，旧日期记录保留明确的保守回退
- [新功能] ETF 大跌策略回测新增固定参数的多区间、跨 ETF 滚动稳健性验证，区分样本内/样本外并汇总通过率、最差窗口和触发覆盖
- [改进] ETF 回测的一键创建活跃策略增加稳健性门禁，当前参数只有同时满足总体与样本外通过率门槛时才可启用
- [测试] 增加滚动窗口划分、跨 ETF 缺失数据、稳健性门槛、Web 创建门禁及参数失效回归测试
- [修复] 稳健性验证保证样本内/样本外均有窗口、按 ETF 身份去重代码别名，并限制单次最多展开 500 个窗口
- [修复] 稳健性验证先划分样本内/样本外交易日边界再生成窗口，避免滚动窗口跨界重叠导致样本外通过率偏高
- [修复] Web 手工策略检查先返回已落库结果并在后台发送通知，增加专用超时与状态对账，避免页面超时但后台已成功触发的假失败
- [修复] 单项策略检查超时后提供无副作用的“重新确认”，后台延迟落库后可在当前页面解除检查锁
- [改进] 策略计划列表默认显示“当前计划”并隐藏已移除记录，历史计划仍可通过状态筛选查看
- [新功能] 新增 ETF 大跌分档策略回测，支持按 250 日回撤设置累计目标仓位并展示收益、最大回撤、资金利用率、触发和等待指标
- [新功能] Web 回测页新增 ETF 策略入口，可将已验证档位一键创建为不依赖账户的活跃指数大跌计划
- [测试] 增加 ETF 分档回测服务、HTTP 合约、Web 参数提交和一键创建计划回归测试
- [新功能] 宽基 ETF 支持每日调度预热和发现页立即预热，日线写入通用 SQLite 缓存并展示历史指标覆盖率、截至日期与旧缓存状态
- [改进] ETF 发现榜与指数大跌策略检查复用同一套 250 日回撤计算；旧缓存可继续展示但不会触发策略档位
- [测试] 增加 ETF 预热失败隔离、旧缓存指标、调度接入、API 与 Web 立即预热回归测试
...
- [改进] 将策略计划不可恢复的关闭操作统一改称“移除”，并明确计划及执行历史仍会保留

<!-- 新条目格式：- [类型] 描述（类型取值：新功能/改进/修复/文档/测试/chore）-->
<!-- 每条独立一行追加到本段末尾，无需分类标题，合并时冲突最小 -->
- [新功能] 新增投资策略计划工作台，支持六类可编辑预填模板、价格/基准回撤执行档位、不可改写的已执行历史、目标仓位投影后的现金纪律、独立每日检查及首次触发 alert 通知。
- [测试] 增加策略计划生命周期、条件评估、通知去重、API 合约、Web 执行轨道及每日任务接入测试。
- [修复] 修正公开行情请求线程与请求头变量同名覆盖的问题，恢复腾讯、新浪和东方财富实时行情 fallback。
...
- [新功能] Web 新增「股票发现」页，可按市场、行业、关键词筛选股票，并通过行情榜单进入分析或问股。
- [新功能] 新增 `GET /api/v1/stocks/rankings` 榜单接口，支持市场、行业、指标、排序方向和 1..100 条限制，并为美股使用核心池。
- [改进] 股票索引兼容旧 10 字段、新 12 字段和对象格式，新增静态行业元数据及 `discover` 分析来源。
- [改进] 股票索引 `popularity` 改为离线生成的真实静态热度分，新增共享批量行情指标 helper 与 `scripts/update_stock_popularity.py` 刷新脚本。
- [改进] popularity 离线刷新在 A 股/北交所主批量行情源失败后，会尝试 Eastmoney 轻量分页 fallback，降低大批量接口被断开时的整市场失败概率。
- [改进] 候选页行情榜单增加 5 分钟前端会话缓存，刷新页面时先展示缓存候选并在后台更新，减少重复等待和外部行情源压力。
- [文档] 补充股票发现页、榜单 API、行业覆盖和美股核心池口径说明。
- [测试] 增加股票索引 parser、榜单路由/排序/状态和 Web 发现页交互测试。
...
- [改进] Agent chat API 返回 session detail context，新增会话 context 保存/移除端点，并校验 `session_id` 与历史报告来源。
- [文档] 明确问股上下文快照、导出/通知暂不包含上下文卡片，以及相关 API 行为。
- [修复] 问股持久上下文后续追问改由会话快照恢复，避免原历史报告删除后再次追问 404，并修复慢速 hydrate 切会话写错上下文与会话列表全量 N+1 查询。
- [修复] 问股初始会话加载不再覆盖历史报告追问新会话，并限制 AkShare 超时后后台挂起线程继续堆积。
- [改进] Web 问股历史报告上下文卡片改为渲染在消息流中、首条追问消息前，并支持展开查看历史摘要与策略点位详情。
- [修复] Web 股票发现页可关注股票列表新增紧凑分页、每页数量选择和表格内滚动，避免长列表撑高页面或后续股票无法浏览。
- [改进] Web 股票发现页将标题、筛选和统计压缩为单一工具区，提升股票列表首屏占比。
...
- [修复] 股票历史 API 增加自然日窗口缓存密度校验，避免首尾日期齐全但中间大面积缺失的稀疏 K 线缓存被误判为完整。
- [新功能] Web 股票发现页新增自选股星标，可从股票列表和行情榜单直接加入或移出 `STOCK_LIST`，并支持「只看自选」过滤。
...
- [改进] 新增腾讯、新浪、东方财富直连的 `public_auto` 行情适配器，覆盖 A 股、北交所、港股和美股个股，并接入超时、熔断、短缓存、目标化批量预取和现有多源 fallback。
- [修复] 公开日 K 增加 OHLC、日期、重复数据、最新日期与窗口密度校验，稀疏或异常结果会继续回退到 YFinance、AkShare、Efinance 等原有数据源。
- [修复] 公开行情统一 A 股/北交所日 K 与实时成交量的“股”口径，并为显式 SH/SZ 代码补齐名称、筹码、板块和基本面身份隔离及响应体硬截止时间。
- [修复] 历史 K 线缓存隔离非默认 SH/SZ 同码证券，美股公开实时报价不再因结构性缺失字段重复请求 YFinance，美股日线按配置的数据源优先级路由。
- [测试] 增加公开行情代码映射、市场字段口径、缓存、fallback、历史数据质量门和管理器路由回归测试。
- [修复] A 股大盘统计的东方财富与新浪 AkShare 调用增加超时、冷却和悬挂调用去重保护，避免单次行情请求长期阻塞后续定时任务。
- [修复] 补充 Agent 模式 LiteLLM 工具调用所需的 `orjson` 运行依赖，避免定时个股分析全部失败而仅发送大盘复盘。
...
- 🐳 **Docker 发布链路整理** — 发布工作流收敛为正式发布与手动补发两条路径，官方 Docker Hub 镜像名统一为 `xf-zhou/daily_stock_analysis`。
...
- 🐳 **Docker 发布工作流收敛** — 更清晰地区分正式发布与手动补发链路，并统一官方 Docker Hub 镜像名为 `xf-zhou/daily_stock_analysis`。
