...
- `price`: the latest valid symbol price. Its observation date must cover the market's latest completed session. Ordinary checks replace a missing, zero, or unverifiable realtime quote with the latest close from a calendar-validated daily-history snapshot. Intraday high-frequency checks require a timezone-aware quote timestamp from a confirmed trading session no more than five minutes before the check, and do not substitute a daily close for a live price;
- `benchmark_drawdown_250d_pct`: drawdown from the benchmark's high over its latest 250 daily bars; the internal loader requests a 550-calendar-day window to cover at least 250 completed sessions. Bars must reach the latest completed session, so stale provider history cannot trigger a step. Intraday checks use the same 250-session window as the backtest: 249 completed daily bars plus the current session. The current-session high comes from the validated live quote when available and conservatively falls back to the current price; no step triggers without a trustworthy live price. A benchmark symbol is required. The unqualified CSI 300 code `000300` resolves as a Shanghai index.

Operators are limited to `lte`, `gte`, and `between`. Plans can use `minute`, `daily`, `hourly`, or `manual` checking. Minute checks currently support mainland China and Hong Kong. Free US minute quotes can be rate-limited during live trading, so US `minute` remains unavailable until a reliable realtime provider is configured. During upgrade, existing US `minute` plans in SQLite are idempotently migrated to `daily`; their status, execution tiers, and capital records remain unchanged, and active plans continue through the daily schedule. High-frequency checks run only in schedule mode, skip sources without provider timestamps, and validate both quote time and evaluation time against the exchange calendar. Closed, stale, or unverifiable sessions do not trigger. Daily analysis and minute checks do not block each other, and plan-check failures do not break reports, notifications, or backtests. Setting `TUSHARE_TOKEN` no longer opts Tushare into realtime polling automatically; list it explicitly in `REALTIME_SOURCE_PRIORITY` when desired.

The independent `schedule` process atomically writes `scheduler_status.json` beside the configured database. Every scheduler loop refreshes its heartbeat, while minute checks record their market set, completion state, evaluated-plan count, newly triggered steps, and errors. The Web strategy-plan page reads `GET /api/v1/investment-plans/scheduler-status` to show scheduler liveness, the latest heartbeat, and the latest minute check. A heartbeat older than 90 seconds is shown as offline, while a graceful exit is shown as stopped immediately. A missing or malformed status file only affects observability and never stops scheduled work. The provided Docker `analyzer` and `server` services already share `/app/data`; custom deployments must point both processes at the same `DATABASE_PATH` directory.

Each plan can also enable or disable trigger notifications. An empty channel selection inherits the global `alert` route, while an explicit selection targets one configured static channel. Automatic checks and single-plan Web checks can notify first-time, not-yet-notified triggers. A Web check returns its committed evaluation before notification delivery continues after the response, and the UI reports that delivery is queued instead of conflating it with the evaluation result. Every step persistently shows **pending**, **sent**, **failed—retry pending**, or **channel unavailable—retry pending**, together with the latest status time and failure reason. Failed sends remain pending for retry, while successfully delivered steps are not sent again. No order is placed automatically.
...
The lifecycle is `draft -> active -> paused/closed`. The Web UI labels the irreversible `closed` action as **Remove** while retaining the plan and its execution history. The list defaults to **Current plans** and hides removed records; select **All statuses** or **Removed** to inspect that history. An active plan must be paused before replacing its execution steps. A triggered step can be marked completed or skipped, or reset directly to pending when it has not been executed. Completed and skipped steps are immutable execution history; remove the old plan and create a new one to revise its checkpoints. Removed plans cannot be reactivated.

Index-crash plans provide a manual execution and review loop without requiring a linked portfolio account. A plan can define **planned capital**, and one-click creation from the ETF backtest carries over the backtest's initial capital. Every monetary value uses the instrument market's local currency, explicitly shown as `CNY`, `HKD`, or `USD`; no FX conversion is performed. After a buy/add tier triggers, the Web UI replaces generic completion with **Record buy** and collects the actual execution time to the second, price, quantity, fee, and optional note; gross amount is calculated from price and quantity. The browser also submits the historical UTC offset and the backend normalizes it to server time. The API rejects date-only, minute-only, and timezone-less execution timestamps. Once recorded, the tier becomes immutable. The service aggregates invested cash, quantity, fee-inclusive average cost, remaining cash, latest valuation, unrealized P/L, actual return, and planned-capital utilization across all recorded tiers. Valuation compares the full plan-check timestamp with the actual execution timestamp, so delayed entry or historical backfill cannot make an older fill look newer than a later check, while a later same-day check still updates P/L and position gates. Pre-upgrade rows that have only an execution date use a conservative fallback: a later check date wins, while the execution price remains preferred on the same date. For an unbound plan, utilization can be compared with the current tier target, and suggested incremental cash is calculated as current plan equity times target weight minus current market value. For an account-bound plan, the tier target remains an account-equity weight and is explicitly shown as a separate basis, so no cross-basis deviation or automatic amount suggestion is provided. Total cost cannot exceed remaining planned cash, planned capital cannot change after the first execution, and strategy type cannot change after activation. Pre-upgrade index-crash tiers that were completed without fill details are marked for backfill; balances, returns, and utilization remain unavailable, and new triggered fills are blocked until those legacy tiers are backfilled. Removed legacy plans also permit this one-time fill-detail backfill; when planned capital is absent, fill cost remains available while cash and utilization stay unavailable. Skip and pre-execution reset keep their existing behavior. This ledger records only user-confirmed broker fills; it neither connects to a broker nor places orders.
...
| `/api/v1/investment-plans/{id}/steps/{step_id}/execution` | POST | Record a manual fill for a triggered index-crash buy/add tier and return its execution review |
...
| `/api/v1/investment-plans/{id}/steps/{step_id}/execution` | POST | Record a manual index-crash ETF fill and return execution-review metrics |
...
| `/api/v1/stocks/etf-history/warmup?force_refresh=true` | POST | Immediately warm the curated ETF daily histories into SQLite and return fresh, stale-cache, and failed outcomes |
