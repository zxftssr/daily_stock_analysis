...
- `benchmark_drawdown_250d_pct`: drawdown from the benchmark's high over its latest 250 daily bars; the internal loader requests a 550-calendar-day window to cover at least 250 completed sessions. Bars must reach the latest completed session, so stale provider history cannot trigger a step. Intraday high-frequency checks calculate current drawdown from the current realtime price and the 250-session high, and do not trigger when that realtime price cannot be validated. A benchmark symbol is required.

Operators are limited to `lte`, `gte`, and `between`; arbitrary expressions and scripts are not accepted. Each plan can use `minute` (intraday high frequency, once per minute), `daily` (the daily scheduled run), `hourly`, or `manual` checking. Minute checking currently supports mainland China and Hong Kong, whose configured free quote routes can provide trustworthy provider timestamps; US plans can use hourly, daily, or manual checking. Minute and hourly checks run only in schedule mode. Minute quote routing skips sources without provider timestamps and continues to later sources. Each evaluation also confirms that both the quote time and the actual evaluation time are within the market's exchange-calendar session, including midday breaks. It makes no quote request when no market is currently open and fails closed for that cycle if the calendar cannot confirm an open session. Daily analysis runs in a separate non-reentrant worker so it does not block minute checks; a new daily trigger is skipped while the previous daily run is still active. New crash plans created from the ETF backtest default to minute checking. Automatic plan evaluation remains independent of AI analysis, market review, and merged report delivery, and it filters plans by current market and configured frequency. Evaluation is fail-open and cannot break reports, notifications, or backtests.
