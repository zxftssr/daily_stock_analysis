...
- `benchmark_drawdown_250d_pct`: drawdown from the benchmark's high over its latest 250 daily bars; the internal loader requests a 550-calendar-day window to cover at least 250 completed sessions. Bars must reach the latest completed session, so stale provider history cannot trigger a step. Intraday checks use the same 250-session window as the backtest: 249 completed daily bars plus the current session. The current-session high comes from the validated live quote when available and conservatively falls back to the current price; no step triggers without a trustworthy live price. US minute checks request Yahoo 1-minute bars only when a verified timestamp is required, use the bar's own New York timestamp, and cache the result for 15 seconds; stale or closed-session bars are rejected by the existing calendar validation. A benchmark symbol is required.

Operators are limited to `lte`, `gte`, and `between`. Plans can use `minute`, `daily`, `hourly`, or `manual` checking. Minute checks support mainland China, Hong Kong, and US plans; US checks use Yahoo 1-minute bar timestamps. High-frequency checks run only in schedule mode, skip sources without provider timestamps, and validate both quote time and evaluation time against the exchange calendar. Closed, stale, or unverifiable sessions do not trigger. Daily analysis and minute checks do not block each other, and plan-check failures do not break reports, notifications, or backtests.
