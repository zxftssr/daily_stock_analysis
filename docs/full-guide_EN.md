...
- **Stock Discovery** - The Web UI can filter stocks by market, industry, and keyword, browse watchable stocks with a compact toolbar, in-table scrolling, and 20/50/100 page sizes, star/unstar stocks directly in the watchlist with a watchlist-only filter, then open daily candlestick charts with cache status, refresh, moving averages, and crosshair OHLC details, or jump into analysis/chat from the list and quote rankings
