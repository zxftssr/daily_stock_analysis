...
class Config:
    ...
    # 实时行情数据源优先级（逗号分隔）
    # 推荐顺序：public_auto > efinance > akshare_em > tushare
    # - public_auto: 腾讯直连 -> 新浪直连 -> 东方财富直连，自动超时/fallback（推荐）
    # - tencent/sina/eastmoney: 固定使用某个公开直连源，便于诊断
    # - akshare_sina: 新浪财经，基本行情稳定，但无量比
    # - efinance/akshare_em: 东财全量接口，数据最全但容易被封
    # - tushare: Tushare Pro，需要2000积分，数据全面（付费用户可优先使用）
    realtime_source_priority: str = "public_auto,efinance,akshare_em"
    # 实时行情缓存时间（秒）
